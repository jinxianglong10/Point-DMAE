...
def dataset_builder_svm(config):
    train_val_loader = torch.utils.data.DataLoader(build_dataset_from_cfg(config.train._base_, config.train.others), num_workers=8, batch_size=128, shuffle=True)
    test_val_loader = torch.utils.data.DataLoader(build_dataset_from_cfg(config.test._base_, config.test.others), num_workers=8, batch_size=128, shuffle=True)
    ...
    def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
        ...
        for name, param in model.module.named_parameters():
            ...
            if config.optimizer.part == 'only_new':  ## only the newly added classifier is learned.
                if ('cls' in name) or ('tfts' in name):     # 不是这里面的根本不会往decay里面加
                # if ('cls_head' in name):
