...
test_transforms_scan = transforms.Compose(
    [
    ]
)
...
test_transforms_model = transforms.Compose(
    [
    ]
)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if 'scan' in args.config:
                train_transforms = train_transforms_scan
            else:
                train_transforms = train_transforms_model
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            if better:
                ...
                print_log(
                    "--------------------------------------------------------------------------------------------",
                    logger=logger)
            print_log('********* best_acc = %.4f *********' % (best_metrics.acc), logger=logger)
            if args.vote:
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger=None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if 'scan' in args.config:
                test_transforms = test_transforms_scan
            else:
                test_transforms = test_transforms_model
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            for kk in range(times):
                ...
                if 'scan' in args.config:
                    test_transforms = train_transforms_scan
                else:
                    test_transforms = train_transforms_model
                points = test_transforms(points)
...
def test(base_model, test_dataloader, args, config, logger=None):
    ...
    with torch.no_grad():
        ...
        acc = 0.
        for time in range(1, args.vote_epoch):
...
def test_rec(args, config):
    ...
    test_only_rec(base_model, test_dataloader, args=args, config=config, logger=logger)





def test_only_rec(base_model, test_dataloader, args, config, logger = None):
    ...
    with torch.no_grad():
        ...
        print_log('[TEST] Losses_block = %.4f Losses_rand = %.4f' % (losses.avg(0),losses2.avg(0)), logger=logger)


def test_only_rec_single(base_model, test_dataloader, args, config, logger = None):
    ...
    with torch.no_grad():
        ...
        print_log('[TEST] Losses = %.4f' % (losses.avg(0)), logger=logger)
